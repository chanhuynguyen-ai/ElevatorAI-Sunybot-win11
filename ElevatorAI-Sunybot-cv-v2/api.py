...
from app.face_recog import extract_embedding
...
def cv_register_face(req: RegisterFaceRequest):
    ...
    if not employee_code or not employee_name:
        raise HTTPException(status_code=422, detail='Can co employee_code/employee_id va employee_name/full_name.')

    person = db.upsert_person(employee_code, employee_name, (req.department or '').strip() or "Kỹ thuật")

    if not config.ENABLE_FACE:
        return {
            "ok": True,
            "message": "Da tao ho so nhan vien, nhung ENABLE_FACE=false nen chua capture embedding. Hay bat ENABLE_FACE=true de nhan dien nhan vien realtime.",
            "person": person,
            "embedding": None,
            "source": "cv_service_register_face_pending",
        }
    if camera_service.face_app is None:
        return {
            "ok": True,
            "message": "Da tao ho so nhan vien, nhung face_app chua san sang nen chua capture embedding. Hay cai insightface/onnxruntime va khoi dong lai CV service.",
            "person": person,
            "embedding": None,
            "source": "cv_service_register_face_pending",
        }
    ...
    if req.snapshot_path:
        path = req.snapshot_path
        if os.path.isfile(path):
            frame = cv2.imread(path)
    if frame is None and req.event_id:
        event_row = db.fetch_event_by_id(req.event_id)
    ...
    if frame is None:
        return {
            "ok": True,
            "message": "Da tao ho so nhan vien, nhung chua co frame camera de capture embedding ngay luc nay. Hay dua mat vao camera va dang ky lai.",
            "person": person,
            "embedding": None,
            "source": "cv_service_register_face_pending",
        }
    ...
    return {
        "ok": True,
        "message": "Dang ky khuon mat thanh cong. Tu bay gio CV co the doi chieu de nhan dien nhan vien nay neu pipeline nhan dien dang bat.",
        "person": person,
        "embedding": saved,
