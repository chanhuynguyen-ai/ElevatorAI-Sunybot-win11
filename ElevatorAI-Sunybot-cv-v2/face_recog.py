...
def _has_cuda_provider(providers):
    for item in providers or []:
        if 'cuda' in str(item).lower():
            return True
    return False
...
def create_face_app():
    ...
    try:
        ...
        ctx_id = 0 if _has_cuda_provider(available) else -1
    except Exception:
        ctx_id = -1
...
def match_face(face_embedding):
    ...
    for r in rows:
        ...
        if score > best_score:
            best_name = r.get("full_name") or r.get("person_code")
