...
const api = {
  ...
  registerFace: async (payload = {}) => {
    const sourceEvent =
      payload?.source_event && typeof payload.source_event === 'object'
        ? payload.source_event
        : {};

    const employeeCode = String(
      payload.employee_code || payload.employee_id || sourceEvent.person_code || ''
    )
      .trim()
      .toUpperCase();

    const employeeName = String(
      payload.employee_name || payload.full_name || sourceEvent.person_name || ''
    ).trim();
    ...
    const normalized = {
      ...
      cam_id: String(payload.cam_id || sourceEvent.cam_id || sourceEvent.camera_id || '').trim(),
      event_id: payload.event_id || sourceEvent.event_id || sourceEvent.id || null,
      track_id: payload.track_id || sourceEvent.track_id || null,
      snapshot_path: String(payload.snapshot_path || sourceEvent.snapshot_path || '').trim(),
      source_event: Object.keys(sourceEvent).length ? sourceEvent : null,
    };

    if (!normalized.employee_id) {
      throw new Error('Thiếu mã nhân viên để đăng ký khuôn mặt.');
    }
    if (!normalized.employee_name) {
      throw new Error('Thiếu họ tên nhân viên để đăng ký khuôn mặt.');
    }
