...
// Bật/tắt bằng biến môi trường để dùng cả Win11 lẫn Jetson Nano.
// VITE_ENABLE_DEMO_LOGIN=1 => bật demo/local fallback
// VITE_ENABLE_DEMO_LOGIN=0 => tắt demo/local fallback
const ENABLE_DEV_DEMO_LOGIN = import.meta.env.VITE_ENABLE_DEMO_LOGIN !== '0';

const DEV_DEMO_ACCOUNT = {
  employee_code: import.meta.env.VITE_DEMO_EMPLOYEE_CODE || 'DEMO001',
  password: import.meta.env.VITE_DEMO_PASSWORD || '123456',
  employee_id: import.meta.env.VITE_DEMO_EMPLOYEE_CODE || 'DEMO001',
  full_name: import.meta.env.VITE_DEMO_FULL_NAME || 'Kỹ thuật viên Demo',
  employee_name: import.meta.env.VITE_DEMO_FULL_NAME || 'Kỹ thuật viên Demo',
  department: import.meta.env.VITE_DEMO_DEPARTMENT || 'Trung tâm bảo trì',
  role: 'maintenance_demo',
  source: 'frontend_demo_local',
};
...
const MAINT_V2_STORAGE_KEYS = {
  authed: 'maint_v2_authed',
  empCode: 'maint_v2_emp_code',
  profile: 'maint_v2_profile',
};
const ENABLE_MAINT_REMEMBER_SESSION = import.meta.env.VITE_MAINT_REMEMBER_SESSION === '1';
...
function parseCellInput(nextValue, previousValue) {
  ...
  const bodyHtml = rows
    .map((row) => {
      const cells = safeColumns
        .map((col) => `<td>${formatCellValue(row?.[col]).replace(/</g, '&lt;').replace(/>/g, '&gt;')}</td>`)
        .join('');
      return `<tr>${cells}</tr>`;
    })
    .join('');
...
function clearLegacyMaintenanceSession() {
  try {
...
function clearMaintenanceSessionV2() {
  try {
    localStorage.removeItem(MAINT_V2_STORAGE_KEYS.authed);
    localStorage.removeItem(MAINT_V2_STORAGE_KEYS.empCode);
    localStorage.removeItem(MAINT_V2_STORAGE_KEYS.profile);
  } catch {}
}

function persistMaintenanceSessionV2(profile) {
  if (!ENABLE_MAINT_REMEMBER_SESSION || !profile) return;
  try {
    localStorage.setItem(MAINT_V2_STORAGE_KEYS.authed, '1');
    localStorage.setItem(
      MAINT_V2_STORAGE_KEYS.empCode,
      profile.employee_code || profile.employee_id || ''
    );
    localStorage.setItem(MAINT_V2_STORAGE_KEYS.profile, JSON.stringify(profile));
  } catch {}
}

function getStoredMaintenanceSession() {
  clearLegacyMaintenanceSession();
  if (!ENABLE_MAINT_REMEMBER_SESSION) {
    clearMaintenanceSessionV2();
    return { authed: false, empCode: '', profile: null };
  }
  const authed = localStorage.getItem(MAINT_V2_STORAGE_KEYS.authed) === '1';
  const empCode = localStorage.getItem(MAINT_V2_STORAGE_KEYS.empCode) || '';
  const profile = readJsonLocalStorage(MAINT_V2_STORAGE_KEYS.profile, null);
  return {
    authed,
    empCode: empCode || profile?.employee_code || profile?.employee_id || '',
    profile,
  };
}
...
export default function Maintenance() {
  ...
  const storedSession = getStoredMaintenanceSession();

  const [authed, setAuthed] = useState(Boolean(storedSession.authed));
  const [empCode, setEmpCode] = useState(storedSession.empCode || '');
  const [currentProfile, setCurrentProfile] = useState(storedSession.profile || null);
  ...
  const [selectedDb, setSelectedDb] = useState('elevator_llm');
  ...
  useEffect(() => {
    clearLegacyMaintenanceSession();
    if (ENABLE_MAINT_REMEMBER_SESSION) {
      const session = getStoredMaintenanceSession();
      setAuthed(Boolean(session.authed));
      setEmpCode(session.empCode || '');
  ...
  useEffect(() => {
    ...
    async function loadTableOptions() {
      if (!authed || !api.dataTables) return;
      try {
  ...
  useEffect(() => {
    ...
    async function loadTableData() {
      if (!authed || !selectedTable) return;
      ...
      setDataLoading(true);
      try {
        const payload = await api.dataTable(selectedDb, selectedTable, 20, 0);
        if (disposed) return;
  ...
  }, [authed, selectedDb, selectedTable, cvEvents, densityData, cvStatus, localUsers]);
  ...
  const healthTiles = [
    { label: 'Camera', value: (cvStatus?.camera_online ?? cvStatus?.online) ? 'ON' : 'OFF', tone: (cvStatus?.camera_online ?? cvStatus?.online) ? 'ok' : 'err' },
  ...
  const applyAuthSession = (profile, successMessage, successDetail) => {
    clearLegacyMaintenanceSession();
    clearMaintenanceSessionV2();
    persistMaintenanceSessionV2(profile);
    setEmpCode(profile?.employee_code || profile?.employee_id || '');
    setCurrentProfile(profile || null);
    setAuthed(true);
    setMssv('');
    addLocalTimeline(
      `Đăng nhập: ${profile?.full_name || profile?.employee_name || profile?.employee_code || 'Kỹ thuật viên'}` ,
      'info',
      profile?.employee_code || profile?.employee_id || 'SYS'
    );
    showToast(successMessage, successDetail);
  };
  ...
  const handleLogin = async () => {
    ...
    const tryLocalLogin = () => {
      ...
      const found = users.find(
        (item) => item.employee_code === employeeCode && item.password === password
      );
      ...
      applyAuthSession(
        {
          ...
          employee_id: found.employee_code,
          full_name: found.full_name || found.employee_code,
          employee_name: found.full_name || found.employee_code,
          department: found.department || 'Kỹ thuật',
          role: found.role || 'technician',
          source: 'frontend_local_fallback',
        },
        'Đăng nhập local thành công',
        'Backend auth chưa sẵn sàng, đang dùng tài khoản test local.'
      );
      return true;
    };
    ...
    if (
      ...
      employeeCode === DEV_DEMO_ACCOUNT.employee_code &&
      password === DEV_DEMO_ACCOUNT.password
    ) {
      applyAuthSession(
        buildDemoSession(),
        'Đăng nhập demo thành công',
        'Bạn đang ở chế độ test local.'
      );
      return;
    }

    try {
      const payload = await api.maintenanceLogin({
      ...
      applyAuthSession(profile, 'Đăng nhập thành công');
    } catch (error) {
      const ok = tryLocalLogin();
      if (!ok) {
        showToast(
          'Đăng nhập thất bại',
          error?.message || 'Sai tài khoản hoặc backend auth chưa bật.'
        );
      }
    }
  };
  ...
  const handleRegister = async () => {
    ...
    if (!cleanUser.employee_code || !cleanUser.password.trim() || !cleanUser.full_name) {
      showToast('Thiếu thông tin', 'Nhập mã nhân viên, họ tên và mật khẩu.');
      return;
    }

    try {
      await api.maintenanceRegister(cleanUser);
      showToast('Đăng ký thành công');
    } catch (error) {
      const nextUsers = saveLocalUser(cleanUser);
      setLocalUsers(nextUsers);
      showToast(
        'Đăng ký local thành công',
        'Backend auth chưa sẵn sàng, tài khoản đã được lưu local để test.'
      );
    }

    setAuthMode('login');
    setEmpCode(cleanUser.employee_code);
    setMssv(cleanUser.password);
    addLocalTimeline(`Tạo tài khoản kỹ thuật: ${cleanUser.employee_code}`, 'info', cleanUser.employee_code);
  };
  ...
  const registerFaceRequest = async (payload) => {
    if (typeof api.registerFace === 'function') {
      return api.registerFace(payload);
    }
    const response = await fetch('/api/integration/cv/register-face', {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      const message =
        ...
        data?.message ||
        data?.error ||
        'Không thể đăng ký khuôn mặt nhân viên.';
  ...
  const runLLMQuery = async () => {
    ...
    try {
      const data = await (api.maintenanceChat || api.chat)(llmInput, {
  ...
  const openRegisterFace = () => {
    ...
    setFaceForm((prev) => ({
      employee_code: (prev.employee_code || profile?.employee_code || profile?.employee_id || empCode || '').trim().toUpperCase(),
      employee_name: prev.employee_name || profile?.full_name || profile?.employee_name || '',
      department: prev.department || profile?.department || 'Kỹ thuật',
      note: unknownCandidate
        ? `Ứng viên từ event ${unknownCandidate.title} lúc ${unknownCandidate.time}`
        : prev.note || 'Đăng ký khuôn mặt nhân viên từ giao diện bảo trì',
    }));
    setShowFaceModal(true);
  };

  const handleRegisterFace = async () => {
    const employeeCode = faceForm.employee_code.trim().toUpperCase();
    const employeeName = faceForm.employee_name.trim();
    const department = (faceForm.department || currentProfile?.department || 'Kỹ thuật').trim();
    const note = faceForm.note.trim();
    ...
    try {
      const payload = {
        ...
        event_id: unknownCandidate?.raw?.event_id || unknownCandidate?.raw?.id || null,
        track_id: unknownCandidate?.raw?.track_id || null,
        snapshot_path: unknownCandidate?.raw?.snapshot_path || '',
        source_event: unknownCandidate?.raw || null,
      };
      const result = await registerFaceRequest(payload);
      addLocalTimeline(`Đăng ký khuôn mặt nhân viên ${employeeCode}`, 'info', employeeCode, cvStatus?.cam_id || 'cam_01');
      showToast(`Đã đăng ký khuôn mặt cho ${employeeCode}`, result?.message || 'Hãy đứng trước camera để hệ thống nhận diện lại.');
      setShowFaceModal(false);
      setFaceForm({ employee_code: '', employee_name: '', department: '', note: '' });
    } catch (err) {
      showToast('Đăng ký khuôn mặt thất bại', err?.message || String(err));
    } finally {
  ...
  const handleCellChange = (rowId, column, value) => {
    setTableRows((prev) =>
      prev.map((row, idx) => {
        ...
        return {
          ...row,
          [column]: parseCellInput(value, row?.[column]),
          _dirty: true,
        };
      })
    );
  ...
  if (!authed) {
    return (
      <div className="maint-auth-screen">
        ...
        <div style={authCardStyle}>
            ...
            <div>
              ...
              <div className="muted" style={{ marginTop: 8 }}>
                {authMode === 'login'
                  ...
                  : 'Tạo tài khoản test local hoặc nối backend auth thật sau.'}
              </div>
            </div>
          ...
          {authMode === 'login' ? (
            <>
              ...
              {ENABLE_DEV_DEMO_LOGIN ? (
                <div style={authNoteStyle}>
                  Demo/local test: <b>{DEV_DEMO_ACCOUNT.employee_code}</b> / <b>{DEV_DEMO_ACCOUNT.password}</b>.  
                  Mặc định màn đăng nhập luôn hiện trước; chỉ bật ghi nhớ phiên khi đặt <b>VITE_MAINT_REMEMBER_SESSION=1</b>.
                </div>
              ) : null}
            </>
          ) : (
            <>
              ...
              <div style={authNoteStyle}>
                Nếu backend <b>elevator_user</b> chưa sẵn sàng, tài khoản sẽ được lưu local để test trên chính máy đang chạy.
              </div>
            </>
          )}
        </div>
      ...
      <section className="maint-top-grid" style={{ alignItems: 'stretch' }}>
        <div className="card maint-camera-card" style={{ minHeight: 590 }}>
          ...
          <div className="camera-stage" style={{ minHeight: 480 }}>
            {cameraVisible && streamUrl && !streamBroken ? (
              <img
                src={streamUrl}
                alt="CV stream"
                className="camera-image-fill"
              ...
              <div className="camera-placeholder-full">
                ...
                <div className="camera-subnote">{cameraVisible ? 'Dashboard vẫn hoạt động dù camera tạm thời lỗi.' : 'Camera backend vẫn chạy để nhận diện và ghi dữ liệu.'}</div>
              </div>
            )}
      ...
      <section className="card data-manager-wide-card">
        <div className="data-manager-head">
          <div>
            ...
            <div className="muted">Hiển thị dữ liệu thật từ API nếu backend sẵn sàng. Fallback local cho test.</div>
          </div>
      ...
      >
          <div
            ...
            style={{
              ...
              background: 'linear-gradient(180deg, rgba(8,18,40,0.98) 0%, rgba(5,12,28,0.98) 100%)',
              boxShadow: '0 28px 80px rgba(0,0,0,0.42)',
              padding: 22,
            }}
          >
            <div
              className="face-register-head"
              style={{
                display: 'flex',
            ...
            >
              <div>
                ...
                <div className="muted">Bấm nút ở góc phải dưới màn hình để mở nhanh form đăng ký và gắn khuôn mặt cho nhân viên.</div>
              </div>
              <button className="btn btn-ghost btn-sm" onClick={() => setShowFaceModal(false)}>Đóng</button>
            </div>

            <div
              className="face-register-candidate"
              style={{
                marginBottom: 18,
                padding: 14,
                borderRadius: 16,
            ...
            >
              <div className="face-candidate-title" style={{ fontWeight: 800, marginBottom: 10 }}>Đối tượng chờ gán nhãn</div>
              {unknownCandidate ? (
            ...
            >
              <label style={{ display: 'grid', gap: 8 }}>
                ...
                <input
                  ...
                  onChange={(e) => setFaceForm((prev) => ({ ...prev, employee_code: e.target.value.toUpperCase() }))}
            ...
            <div
              className="face-register-actions"
              style={{
                display: 'flex',
