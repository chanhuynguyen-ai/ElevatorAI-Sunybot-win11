...
from elevator_agent.agent.memory_store import ConversationMemoryStore
from elevator_agent.agent.planner import Planner
from elevator_agent.agent.safety import SafetyGuardrails
from elevator_agent.agent.tool_registry import ToolRegistry
from elevator_agent.schemas import Citation, ChatResponse, ToolTrace
...
class AgentRuntime:
    ...
    def run(
        ...
        if precheck.get("status") == "emergency" and not any(call.tool_name == "get_elevator_status" for call in plan.tool_calls):
            from elevator_agent.schemas import ToolCall
