...
def _best_effort_register_person(conn, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    row: Dict[str, Any] = {}
    employee_id = payload.get("employee_code") or payload.get("employee_id") or payload.get("person_id")
    employee_name = payload.get("employee_name") or payload.get("full_name") or payload.get("person_name")
...
def register_face(req: RegisterFaceRequest):
    ...
    if employee_code and not employee_name:
        user = _find_maintenance_user_db(employee_code) or _find_maintenance_user_file(employee_code)
        if user:
            employee_name = user.get("full_name") or user.get("employee_name") or employee_code

    if not employee_code or not employee_name:
        raise HTTPException(status_code=422, detail={
            "message": "Can co employee_code/employee_id va employee_name/full_name de dang ky khuon mat.",
            "employee_code": employee_code,
    ...
    for url in upstream_candidates:
        try:
            ...
            return {
                ...
                "employee_name": employee_name,
                "data": data,
                "message": data.get("message") if isinstance(data, dict) else "Da gui dang ky khuon mat sang CV service.",
            }
        except Exception as exc:
